...
>
> **Orchestrator + specialized worker** is a pattern, not a framework. Whether you use the Claude Agent SDK, Claude Managed Agents, or direct Anthropic API calls — the architecture is the same: one agent routes and coordinates, specialized agents execute with focused context and tool scope. The framework changes; the orchestration pattern doesn't.
