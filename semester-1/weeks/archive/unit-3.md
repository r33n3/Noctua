...
- Understand the six AIUC-1 domains and their application to security systems
...
- Connect AIUC-1 domains to NIST AI RMF governance stages
- Evaluate how responsible AI principles reduce organizational risk and regulatory exposure

#### Lecture Content

The **AIUC-1 Standard** (2025) represents the first security, safety, and reliability standard specifically designed for AI agents, developed by the Artificial Intelligence Underwriting Company (AIUC) in collaboration with MITRE, Cloud Security Alliance, Stanford, MIT, Cisco, and Orrick. Originally developed because enterprises couldn't reliably assess AI vendor security, this standard is broadly applicable to security systems across all industries.

> **🔑 Key Concept:** The AIUC-1 standard bridges the gap between abstract AI ethics and concrete security engineering. Rather than treating ethics as separate from security, it embeds ethical requirements into every stage of AI system deployment: design, testing, deployment, and monitoring.
...
The AIUC-1 domains emerged from real incidents in the financial sector where AI systems caused significant harm:
...
- **Flash Crash (2010):** Automated high-frequency trading (HFT) algorithms amplified market volatility, causing a temporary $1 trillion loss in market value in minutes. The algorithms operated with insufficient safeguards and no human oversight, demonstrating the need for resilience principles and human-in-the-loop controls.
...
Organizations implementing AIUC-1 domains realize tangible benefits:

- **Regulatory Compliance:** The EU AI Act (2024) requires risk-based governance and transparency. Organizations following AIUC-1 domains are better positioned for compliance.
...
- Audit an existing security tool or system against the six AIUC-1 domains
...
- Connect OWASP Top 10 to the AIUC-1 Standard and NIST AI RMF
...
| OWASP Risk | AIUC-1 Domain | Connection |
|---|---|---|
| Excessive Agency | B. Security (B006) + E. Accountability | Limit system access + audit trail for all agent actions |
| Insufficient Guardrails | C. Safety (C001+) | Constraints prevent unsafe behavior |
| Insecure Tool Integration | B. Security (B006) | Tool inputs/outputs must be validated; least-privilege access |
| Lack of Output Validation | D. Reliability | Outputs must be accurate before use |
| Prompt Injection | B. Security (B001, B002, B005) | Adversarial robustness testing, input detection, real-time filtering |
| Memory Poisoning | D. Reliability | Data integrity must be maintained |
| Supply Chain Vulnerabilities | B. Security (B008) | Protect model deployment environment |
| Insufficient Logging | E. Accountability | Decisions must be auditable |
| Over-reliance on AI | D. Reliability + E. Accountability | Continuous validation + human oversight mechanisms |
| Inadequate IAM | B. Security (B007) | Enforce user access privileges to AI systems |
...
[ ] AIUC-1 Standard
    [ ] A. Data & Privacy: Data governance, consent, PII protection
    [ ] B. Security: Adversarial robustness, input filtering, access control
    [ ] C. Safety: Harmful output prevention, risk taxonomy, pre-deployment testing
    [ ] D. Reliability: Validation, drift detection, graceful degradation
    [ ] E. Accountability: Audit trails, decision logging, explainability
    [ ] F. Society: Bias testing and fairness monitoring
...
## AIUC-1 Standard
- [ ] A. Data & Privacy: [specific questions to ask]
...
```
# AIUC-1 Domains → Technical Controls Mapping

| Domain | Definition | Technical Control | How to Verify |
|--------|-----------|-------------------|---------------|
| A. Data & Privacy | Data governance and PII protection | Data minimization, consent management, output inversion testing | Data flow audit; inversion test results |
| B. Security | Adversarial robustness and access control | Adversarial testing (B001), input filtering (B005), least-privilege (B006) | Third-party test results; permission audit |
| ... | ... | ... | ... |
